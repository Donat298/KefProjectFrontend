<template>
  <div>
    ...
    >
      ...
      <div style="display: flex; align-items: center; justify-content: center;">
       You won <strong style="margin: 0px 5px;">{{ GameResult.wonMsg }}</strong> 
        <img
          :src="currencyImage"
          style="width: 17px; height: 17px; margin-right: 5px; "
        /> <strong> {{ GameResult.currency }}!</strong> 
      </div>
    </v-alert>
   

    <v-alert
      v-if="GameResult && GameResult.lose && !errorMsg" 
      border="end"
      variant="tonal"
      color="red lighten-1"
      class="mx-auto"
  ...
  export default {
    ...
    computed: {
        currencyImage() {
          return this.currencyImages[this.GameResult.currency];
        }
      }
  }
  </script>
